...
const totp = require('./totp'); // eslint-disable-next-line import/newline-after-import
...
class Avanza extends EventEmitter {
  ...
  authenticate(credentials) {
    ...
    if (!(this._authenticationTimeout >= MIN_INACTIVE_MINUTES && this._authenticationTimeout <= MAX_INACTIVE_MINUTES)) {
      return Promise.reject(new Error(`Session timeout not in range ${MIN_INACTIVE_MINUTES} - ${MAX_INACTIVE_MINUTES} minutes.`));
    }
  ...
  getInstrument(instrumentType, instrumentId) {
    const path = constants.paths.INSTRUMENT_PATH.replace('{0}', instrumentType.toLowerCase()).replace('{1}', instrumentId);
    return this.call('GET', path);
  }
  /**
  ...
  search(searchQuery, type, limit = 10) {
    const options = {
      query: searchQuery,
      searchFilter: {
  ...
  call(method = 'GET', path = '', data = {}) {
    ...
    return new Promise((resolve, reject) => {
      if (!this._authenticated) {
        ...
        }).then(response => resolve(response.body)).catch(e => reject(e));
      }
    });
  }
